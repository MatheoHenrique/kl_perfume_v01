...
def novo_produto(request):
    if request.method == "GET":
        ...
        if Produto.objects.filter(nome=nome, marca_id=marca, tipo_id=tipo) is not None:
    ...
    if request.method == "GET":
        ...
        for dado in campos:
            if len(dado.strip()) == 0:
                messages.add_message(request, constants.ERROR, 'Preencha todos os campos.')
                return render(request, 'novo_cliente.html')
        if Cliente.objects.filter(usuario_id=request.user.id, nome=nome, sobrenome=sobrenome) is not None:
